...
下面的截图来自 **Virtual Drive Center 2.0.13** 的实际运行界面，用于展示项目的核心交互流程。截图中的镜像名称、挂载 ID 与存储统计已做示例化处理，避免公开个人环境信息。

### 1. 首页 / 映像库

首页集中展示映像数量、当前挂载、ISO / IMG 分类、NAS 存储状态以及映像库操作入口。用户可以从这里进入检查、校验、挂载和卸载流程。

![Virtual Drive Center 首页与映像库](docs/images/dashboard.png)

*图 1：Dashboard 与 Image Library。*

### 2. 多分区 IMG 选择

对于带 MBR / GPT 分区表的 IMG，系统会列出检测到的分区、文件系统、卷标和容量，用户选择需要访问的分区后再执行挂载。

![IMG 多分区选择界面](docs/images/img-partitions.png)

*图 2：多分区 IMG 的分区选择界面。*

### 3. 挂载完成与 DSM File Station 路径

挂载成功后，页面会同时显示 NAS 真实路径和 DSM File Station 路径，便于从 Web 管理界面切换到 DSM 文件管理器继续读取、复制或写入文件。

![映像挂载成功与 DSM File Station 路径](docs/images/mount-success.png)

*图 3：挂载成功结果与 File Station 访问位置。*
